# ADR-050 â€” The GPU cascade ships: depth 30, narrowed by the shipped graph, fused with a second opinion

Date: 2026-08-22 Â· Status: **Accepted** Â· Owner: M0c Session N

## Context

`RerankPlan::{Shipped, Cascade}`, `CASCADE_SLATE = 30`, `CASCADE_NARROW = 10`, `RRF_K = 60`,
`FUSION_GRAPHS` and `ScoredCandidate::fusion_rank` have existed since M0c Session M2 â€” with **no
...
Â§5.7's 300 ms), which is why the scaffolding sat behind an explicit "NOT WIRED" warning rather
...
4. *Missing L-6 fixture.* Generated from HuggingFace (`cross-encoder-reference-ft-session-j-L6.json`)
   and enforced by a Rust test through `load_fusion_member`, plus per-graph batch-invariance â€”
...
3. **Announce and stamp.** The plan prints its own startup line (`rerank plan â€¦`) and lands on
...
4. **The fusion is ranks, not scores.** RRF over the two rankings of the narrowed ten, ties by
   candidate index ascending â€” replicating the offline tool exactly, where adjacent swaps are
...
| check | result |
|---|---|
| fit + held-out controls, pinned CPU | 0.7555 / 0.6725 EXACT â€” the shipped path is untouched |
...
* GPU deployments inject from a strictly better measured configuration; CPU-pinned published
  numbers do not move. `repro` across differing card states can now differ by PLAN â€” the same
...
* Held-out narrowing retention measures **0.9777 (5 cases)**, materially worse than fit's 0.9956 â€”
...
* Supersedes nothing; implements what `RerankPlan`'s own doc always described. The warning
  "NOT WIRED â€” nothing reads this" is now false and has been removed by the wiring itself.
