...
1. **[`ADR-032`](adr/ADR-032-approved-egress-and-the-blast-radius.md) is still
   `Status: Accepted 2026-08-31 by the human` — **and still not built**, and its decision is fully built.** It is the only
...
3. **`bash` and egress — ruled on environmentally, never decided.** The adjudicator's egress section
   ...
   **A PROPOSED ANSWER NOW EXISTS AND IT IS NOT AN ACCEPTED ONE (2026-08-31).**
   [`ADR-070`](adr/ADR-070-one-sandbox-per-team-and-it-wraps-bash.md) proposes exactly the containment
   shape this entry names: an AppContainer token whose capability array is **NULL**, so the shell holds
   neither `internetClient` nor `privateNetworkClientServer`, and Windows Filtering Platform drops the
   connect keyed on the package SID. There is no spelling of `curl` that acquires a capability the
   token does not hold, which is why it is containment rather than the filter §8.1 rules out. It also
   answers the loopback wrinkle this entry raises rather than inheriting it: **loopback is blocked for
   AppContainers by default**, and the documented exemption, `CheckNetIsolation LoopbackExempt`,
   **requires admin** — so an agent cannot grant it to itself, and *"allow loopback"* is not a hole
   anyone has to open. **None of it is built and the ADR is `PROPOSED`**, so this entry is not closed;
   what changes is that the triage's question gets narrower, because the shell's egress becomes a
   property of the token rather than a rule the adjudicator has to be taught about a tool that declares
   no `Url`. **It does not touch `web`'s allowlist** — ADR-070 §5 says so in the negative, and item 7
   below carries why. **And the legitimate use this entry names is the part that is UNVERIFIED**:
   whether two processes inside the same AppContainer reach each other over loopback is ADR-070 §4.3's
   second unverified item, so *"run a dev server and curl it"* is not yet known to work inside the box.
...
7. **[`ADR-070`](adr/ADR-070-one-sandbox-per-team-and-it-wraps-bash.md) is `PROPOSED`, and it gates
   the session the human has already named as his next one** — *"deploy a PI team in a sandbox and
   observe it"*. The team half of that is built (`6e01c37`, M3 Session C below); **the box is a design
   with no code in it.** Two things are the human's here and they are different questions. **The first
   is the ADR**, which amends **ADR-002** narrowly — it restores a kernel backstop for `bash` alone, on
   a path that never had one — and makes **ADR-026**'s `Irreversible` ceiling conditional on a run's
   box rather than absolute. Both are §13 territory, and ADR-070 §3 is explicit that *"lift the
   escalation"* and *"build the box"* are **one change, not two**: lifting first does not trade a little
   security for speed, it removes the only control that path has. **The second is not an approval but a
   measurement nobody has taken.** Whether MSYS2 / Git Bash survives AppContainer's redirected object
   namespace is **UNVERIFIED**; ADR-070 §4.3 names it the largest risk in the design and a one-hour
   spike rather than an argument. A no there does not kill the design — the fallback is a native shell
   in the box, or a separate unprivileged account — but it selects what gets built, so it is answered
   before anything is built on it.

   **Ranked at the top by cost and left at the bottom by number, deliberately.** Nothing else on this
   list blocks a session the human has said he wants next. The numbers were not shuffled to say so,
   because [`DECISIONS.md`](DECISIONS.md) cites these items **by number** — renumbering a list another
   document points into is instance #14's shape aimed at a cross-reference, where the citation still
   resolves and quietly names something else.
...
| **C** | The tree and the typed upward channels, **together** — five agent levels, escalation to the user, harness-rendered TERMINATE, budget grants with envelopes. Together, because shipping the hierarchy first and the channels after ships the laundering path alone. **C also carries three things nothing else owns, and each is a prerequisite for D rather than a nicety.** (1) **THE MODEL-DRIVER SEAM.** `Daemon::turn` builds its `Box<dyn ModelDriver>` inline — declared at `daemon.rs:1992`, returned from three provider arms at `:2145`, `:2240` and `:2358` — and hands it to `Ports` at `:2738`; `turn` is private (`:1730`) and neither public door — `ask_streaming_with` (`:1682`), `resume_streaming` (`:1698`) — takes a driver. *(Line numbers re-verified 2026-08-30 at `e1de06d`. Five of the seven citations in this row had drifted, including a `resume` that is spelled `resume_streaming` — instance #14's family aimed at a roadmap row, so re-verify before citing rather than trusting these.)* So no test can drive a real daemon turn with a scripted model, and **`daemon.rs`'s injected-memory push at `:2631` — the only production line the whole layer-3 chain runs through — has zero coverage**: laundered to `UserAsserted` or deleted outright, the whole `marlowe-daemon` crate stayed green (`runs/m3-mutation/finding1*.txt`). The barrier is not assertion strength — provider selection `return`s at `:1802` when `Availability::probe` reports no model, so nothing in-process reaches `:2629` at all. `ModelDriver` is **not** pinned in `CONTRACTS.md` (ADR-060), so the seam is ordinary crate work, not a contract act. **A loadable reranker is a second, independent precondition** — see the paragraph below this table. (2) **`Run::root` is rebuilt every turn** (`daemon.rs:2486`) at `trust_floor: UserAsserted` (`marlowe-loop/src/run.rs:680`); only `Run::from_checkpoint` restores a latched floor, so ADR-023's latch is **per turn, not per conversation**, and CLAUDE.md's prescribed *"across two turns"* probe would measure **re-injection**. **This was already found AND answered on 2026-08-12** — [`SECURITY-AUDIT.md`](SECURITY-AUDIT.md) §8 states it as *"the latch belongs on the session, not the Run"*, seventeen days before B2 re-derived it as open. Record that answer in `DECISIONS.md` before D depends on either, or argue against it explicitly; **do not re-open it as though it were undecided**, which is what this row said until 2026-08-29. (3) **RED-TEAM PASS 1 RUNS AT THE END OF THIS SESSION** — [`REDTEAM-SESSION.md`](REDTEAM-SESSION.md), injection only. C is what ships the control that makes a zero mean anything: [`M3-DESIGN.md`](M3-DESIGN.md) §9.1's **arm A8** (fully typed / typed + one validated sentence / **free text, expected to fail**), so A8's three arms are built with the channel work rather than retrofitted. End of C is the last cheap moment to find out that typed upward containment is decorative, because D and E are both built on the assumption that it is not. **(4) THE ROLE FIELD ON `SpawnRequest`, ADDED 2026-08-30.** The human specified four user-configurable model roles with the spawner naming which one it wants — see [`AGENT-DIRECTORY.md`](AGENT-DIRECTORY.md) §2, whose Ollama defaults are `marlowe-dawn:9b-super` (secretary), `marlowe-mini:4b-super` (agent) and `marlowe-mini:2b` (extractor), sized to **co-reside** rather than swap. The *window* is Session G. **The field is C's**, because C is what pins the spawn shape: `SpawnRequest` today carries `task`, `contract`, `orphan`, `share`, `grant_tokens`, `tools`, `tools_declared`, `reads_untrusted` and **no role**, and CONTRACTS §5 pins it. Ship C without the field and G is a pinned-contract change instead of a window. **Do not invent the fourth role** — four were specified and three named; that is the human's. And note the security question §3 raises and does not answer: if a spawner names a model, is that name a **target** under ADR-023? A downgrade and an upgrade are both attacker-useful, and a model name is not in layer 3's list. **(5) ADMISSION CONTROL ON SPAWN, ADDED 2026-08-30 — see [`AGENT-DIRECTORY.md`](AGENT-DIRECTORY.md) §2a.** Agents are kept warm; as many models as fit go on the GPU and **never a CPU split**; a spawn that cannot fit **waits for one of the same type to finish**. That is a spawn-path decision, so it is C's even though the configuration window is G. **`RunStatus::Queued` already exists in the pinned §5 enum** (`run.rs:647`, `:670`; rendered at `roster.rs:214`) so nothing in the contract has to move — but it currently means *constructed, not yet started*, a transient, and capacity-waiting may last minutes. Rendering both as one word leaves the roster unable to say whether anything is wrong, which is ADR-032 §2's *"`DenyAll` and an empty allowlist behave identically and must stop being the same thing"* in a second place. **And the capacity model is unmeasured**: three same-role workers share one set of weights and multiply the KV cache (`NUM_PARALLEL`, which also divides the context window), while three different roles multiply weights (`MAX_LOADED_MODELS`, unset). Read `/api/ps` before designing the queue or it will be built against the wrong bottleneck | **IN PROGRESS 2026-08-30 — and five of this row's own claims are now settled or false. (1) THE SEAM IS BUILT** (`b44c9f4`): `Daemon::ask_streaming_with_driver` enters `turn` **before** provider selection, and the injected-memory push is mutation-covered in both directions — *"has zero coverage"* is **false as of today**. **(2) THE LATCH SCOPE IS RECORDED AND DEFERRED** by the human, with its deadline named — **before Session D ships, not after** — because D is what makes the per-turn reset live. `DECISIONS.md`, 2026-08-30. **(3) RED-TEAM PASS 1 RAN AND PUBLISHED NO ASR**, which is its result rather than a failure to finish: the metric conflated obeying an injection with faithfully reporting one, two runs of the identical configuration gave opposite orderings, arm (c) can carry a tool note instead of the child's prose, and `marlowe-red:9b` never called `web` on 9 of 12 cells — without the positive control the **unsafeguarded** model would have reported better containment than the safeguarded one. **A8 is unanswered in either direction, so D and E still rest on an untested assumption.** `runs/m3-c/redteam/PASS1-REPORT.md`. **(4) THIS ROW'S ROLE TABLE IS WRONG AND ITS "FOURTH ROLE" NEVER EXISTED.** The defaults named above — secretary/agent/extractor — were an artifact of a table that rendered a capability ladder as roles-by-function. Corrected by the human 2026-08-30: **Secretary + Agent-High/Medium/Low** (`marlowe-dusk:27b-super` AAII 52 / `marlowe-dawn:9b-super` 22 / `marlowe-mini:4b-super` 20). **Nothing was unnamed, and three sessions treated naming it as a blocker.** A role is a **slot, not a model** — two roles resolving to one model is the ordinary case. **(5) THE CAPACITY MODEL IS MEASURED, and the queue's only sensor is unreliable**: `/api/ps` reported `size_vram` **1,631 MiB** for the 27b, stable across three polls, while the card moved 8,856 → 15,415 MiB used — a **7.6× under-report** — and `size == size_vram` there, so the standard CPU-split test reads **GPU-ONLY** on the model most likely to split. **ADR-067 keys its refusal branch and its "never CPU-split" guarantee on exactly those two fields** and must be re-derived. Co-residency is the target and assumes larger cards or smaller models; this card evicts, which is expected. **STILL OPEN:** the five levels and the §2.3 escalation record (ADR-064, ADR-065 — §13-authorized by the human 2026-08-30 and in progress), and admission control. **Seven ADRs, ADR-063–069, are written and all `PROPOSED`**. **(6) §1.2's TOOL RULE IS REVERSED — BUILT AND GREEN, `6e01c37`, 2026-08-31.** M3-DESIGN §1.2 read *"masters hold no working tools, structurally"*, argued as *"a master with an `edit` tool will eventually edit… because it is capable and the work is right there."* **The human reversed it.** The PI is the senior researcher who does the hardest part himself and spawns assistants and interns because the job is larger than one context — not a boss kept away from the work — and forbidding the AAII-52 model to touch the work spends the best model in the ladder on coordination. The requirement is [`PI-MODEL.md`](PI-MODEL.md). **What decided it is that the rule bought less than it looked like:** `SpawnRequest.task` is an `ArgumentRole::Payload` and `composes_spawn_targets` never checks it, so a toolless master still wrote the task for a worker that held `edit` — **it displaced the actor one hop without adding a check.** **What it costs is recorded rather than glossed.** §1.2's real argument was containment: a master reads every worker's report, so it holds the most attacker-exposed context in the tree, and a child's note crosses in at `AgentInferred` — **above** `blocks_composed_targets`'s threshold. That is [`SECURITY-AUDIT.md`](SECURITY-AUDIT.md) finding #1, **already open**, and this moves the product onto it. **The blast radius does not widen in what can be done, only in how well-aimed it is** — the reversal made nothing safer and the ADR says so. **One refusal survived and nearly died by accident:** `MANAGEMENT_TOOLS` withheld working tools *and* `ask` under one whitelist, so deleting it for the first would have silently dropped the second. `ask` is now refused by name — `ProfileError::OnlyATopAgentMayAsk` — because only a top-agent reaches the user; `MANAGEMENT_TOOLS` and `ProfileError::MasterHoldsWorkingTool` are **deleted**. Green on the three crates it touched (`marlowe-loop`, `marlowe-provider`, `marlowe-tools`): **416 passed, 0 failed, 6 ignored across 41 binaries**, `runs/m3-c/pi/suite.txt` — a **scoped** run and not a workspace count. **(7) THE SANDBOX IS DESIGNED AND NOT BUILT, AND IT IS NOT C's.** [`ADR-070`](adr/ADR-070-one-sandbox-per-team-and-it-wraps-bash.md) is `PROPOSED`, **DESIGN ONLY, NO CODE**; it has its own row, **H**, below |
| **H** | **One sandbox per team, and it wraps `bash`** — [`ADR-070`](adr/ADR-070-one-sandbox-per-team-and-it-wraps-bash.md), a new crate `marlowe-sandbox` and one call site in `marlowe-exec`. One AppContainer per top-agent team: full freedom inside, and no way out. The human's requirement is *"agent teams → full access to their own sandbox, securities lifted so they can go wild on their research"*, under *"assume the worst — a git worktree can be left, a sandbox cannot."* **The finding that makes this small is that only ONE thing leaves the daemon process.** `web`, the model call, the journal and every file tool execute in-process (`crates/marlowe-exec/src/lib.rs:1999-2001`); `spawn_shell` is the only `Command::spawn`. So the box wraps **`bash` alone, nothing else moves**, and **research is untouched** — `web` is harness-executed today and stays that way, so a boxed team fetches pages exactly as it does now. The asymmetry is free: the daemon keeps its own token, so the harness reaches into the team folder while the box cannot reach out. **Two facts were verified on this machine by `icacls` and are why this is cheap:** `C:\Users\matth` carries **no `ALL APPLICATION PACKAGES` ACE**, so the user's profile is denied to an AppContainer by Windows' own default ACLs with no code written, while `C:\Program Files\Git` and `System32` **do** carry it, so Git Bash and the system DLLs load; and `CreateAppContainerProfile` needs **no elevation**. **Network denial is the kernel, not a filter:** a token built with a **NULL** capability array holds neither `internetClient` nor `privateNetworkClientServer`, WFP drops the connect on the package SID, **loopback is blocked for AppContainers by default**, and the exemption requires admin so an agent cannot grant it to itself. **A worktree is provisioning, not containment** — `bash` consults no `PathScope`, so a compromised agent walks out of a nominated directory without trying; that correction is the human's (`runs/m3-c/sandbox/CORRECTION.md`) and it is why the mechanism has to be OS-enforced. **STEP ONE IS A SPIKE, NOT AN IMPLEMENTATION:** whether MSYS2 / Git Bash survives AppContainer's redirected object namespace is **UNVERIFIED**, it is the largest risk in the design, and a no there selects a different backend — a native shell in the box, or a separate unprivileged account — rather than killing it. **What H must not be read as retiring: egress.** The box contains **damage**, not **disclosure**: `web` is harness-executed and outside the box, the box holds a copy of the user's source, and the allowlist is the only thing between that and an attacker-named host. **Marlowe is not boxed and his securities stay heavy**, precisely because he has the machine. Two hard constraints from the human bind the build (`runs/m3-c/sandbox/HARD-CONSTRAINTS.md`): **it must never log the user out**, which rules out session switching and any Job Object whose kill scope could reach the daemon; and **nothing verifies the box by running a destructive command** — escape is proved by **reaching something harmless you should not be able to reach**, never by destroying something. The acceptance probe runs with the harness's own path scoping **deliberately disabled**, because with it left on every probe passes whether or not the sandbox exists, which is instance #15 aimed at the single claim the design rests on | **NOT STARTED. NOTHING IS BUILT. GATED ON THE HUMAN.** ADR-070 is **accepted (2026-08-31) and unbuilt** and amends ADR-002 and ADR-026, both §13, so the session cannot open until it is accepted — *Waiting on the human* item 7. **`bash`'s `Irreversible` escalation IS the current sandbox for that path**, so *"lift the escalation"* and *"build the box"* are **one change, not two** (ADR-070 §3), and H may not be split into a lift now and a box later |
...
**WHERE H GOES, AND WHY IT IS NOT C's, D's OR E's — ADDED 2026-08-31.** The sandbox arrived as a
design during C and belongs to none of the rows that existed when it did. It is **not C's**: C ships
the tree and the typed upward channels, and a box changes neither — the PI tool reversal landed in C
because a level's tool set is a property of its profile, but the container is an OS mechanism in a new
crate with one call site. It is **not D's**: scoped memory is a memory-topology change that shares no
surface with a process token. It is **not E's**: meetings need the tree underneath them, not a box.
And it is **not G's**, which is last and decides which model a role loads.

**It goes immediately after C and before D, and the reason is the session the human has named next:**
*"deploy a PI team in a sandbox and observe it"* — a demo, not a build, and it needs both halves. The
team half exists as of `6e01c37`; the box does not exist at all. Put H after D and E and that demo
waits on two milestone-sized sessions that buy it nothing. Put it after C and it runs on the first
configuration that can carry it. **The second reason is the spike.** H's first step is a one-hour
measurement whose answer selects the backend, and a measurement that can change a design is worth
taking while there is still M3 left to absorb the answer — not at the end, where it arrives as a
surprise. **H is the second row out of alphabetical order, and that is this table's own convention:**
letters are labels rather than a sequence, and F is the precedent.

**Nothing in this placement schedules H.** ADR-070 is **accepted (2026-08-31) and unbuilt**, it amends two §13 ADRs, and this is a
proposal about ordering — not an approval, not a start date, and not a claim that any part of the
sandbox is built. If the human accepts it, H is the next session; if he does not, the demo he named
cannot run, and the reason is recorded here rather than discovered at the keyboard.
