...
| **B** | The compaction stamp and the trim marker — **both, before any upward channel is wired.** CLAUDE.md names them and M3-DESIGN §8 explains why they go live together with layer 3 | **HALF DONE, ON A BRANCH.** E5 (the compaction stamp) and F1 (the trim marker) are fixed on `m3-ingest-live` at `9639807`, one commit ahead of `master` and nine behind it — **rebase, do not rewrite**. `ingest` is still NOT wired: `grep -rn "ingest("` shows one production caller, `crates/marlowe/src/adapter.rs:346`, the eval adapter. So the layer-3 latch remains **unreachable in the shipped daemon**, which is the actual subject of the session. Neither probe exists |
