...
| [**ADR-051**](adr/ADR-051-skills-and-progressive-disclosure.md) | **`SKILL.md` loads and `use` runs.** `find_skill` IS `use` — registered since Session A with the two parameters the job needs. The body is a reference, never a field: progressive disclosure enforced by the type. An installed skill is `UserReviewed`, which is what makes §7.1's own example loadable. The front-matter parser refuses every construct outside its subset rather than guessing. **The ranking is lexical, not semantic** — decided, not deferred: the memory system's rankers were tuned on conversations, and carrying that measurement onto skill descriptions is the scoped-measurement family. §5 records the cost and a deferred experiment gated on a real skills library |
| [**ADR-052**](adr/ADR-052-mcp-servers-are-trusted.md) | **MCP servers are TRUSTED and their OUTPUT is not.** A human decision: installing a server is the user's authorization, and the agent cannot install one. Descriptions reach the model as ordinary prose; results are `UntrustedContent` and go through layer 1 unchanged. Descriptions are pinned at install and a change re-asks by name. Stdio only — layer 4 is not shipped, and `marlowe-mcp` has no socket in its dependency graph. `Description::trust()` is deleted rather than left with a test for its only caller |
| [ADR-053](adr/ADR-053-what-a-checkpoint-must-carry.md) | What a checkpoint must carry, and why a step number is not it | M3 A | A resume rebuilt through `Run::root` restarts the trust floor at `UserAsserted` — a daemon restart doing what trimming the untrusted block used to do. Also: orphan policy enforced by amending the child's checkpoint, budgets granted rather than sliced, and a second listener so steering from outside is real |
...
| [**ADR-058**](adr/ADR-058-write-is-its-own-tool.md) | **`write` is its own tool, and the exposure budget moves to 12 -> 13.** `edit` was two tools wearing one name, separated by an OPTIONAL parameter. Told to write a file the model reached for the SHELL -- `cat > f << 'EOF'`, journal seq 4806 -- because no builtin was named for the verb, then picked `edit`'s wrong mode: six calls, three minutes, a zero-byte file. Three attempts to fix it in prose came first and none removed the choice; the ambiguity was in the shape. Both tools now have every parameter required and no mode. The cap moved rather than the MCP allowance, because a fix to Marlowe's own surface must not shrink what a user's server may offer -- `composition_root.rs` asserts the **two slots**, not the number |
| [**ADR-059**](adr/ADR-059-grep-searches-contents.md) | **`find` becomes `grep`, on a real regex, and the walk stops disappearing into `target/`.** The rename is a bug fix: `SHELL_DESCRIPTION` asserted BOTH readings of the word in one paragraph -- the unix list promising `find` matches NAMES, the routing sentence promising the inverse -- and the repo's own probe elicited a contents search with the prompt *"Find every occurrence of TODO"*. The larger half is measured: `collect` had no skip list, `target/` holds ~162k files, and `grep(".")` filled all 2,000 walk slots with build artifacts and never reached `crates/` -- `0 results · 927 files`, reported as a fact about the workspace because `find`, unlike `glob`, never checked whether the cap had bitten. `WALK_SKIP` is one definition the daemon's map builder and the walk both read. `regex` over `fancy-regex`: linear time is a property of the crate, not of a timeout. Four parameters, six rejections, and an undeclared argument is REFUSED by name |
